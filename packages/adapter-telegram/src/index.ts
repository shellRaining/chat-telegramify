...
import { renderTelegramRichText, type TelegramRichEntity } from "telegramify";
import {
  ...
  implements Adapter<TelegramThreadId, TelegramRawMessage>
{
  ...
  private async downloadFile(fileId: string): Promise<Buffer> {
    ...
    if (file.file_path.startsWith("file://") || isAbsolute(file.file_path)) {
      const localPath = file.file_path.startsWith("file://")
        ? fileURLToPath(file.file_path)
        : file.file_path;
      const mappedPath = this.mapLocalBotApiFilePath(localPath);
      return readFile(mappedPath ?? localPath);
    }
  ...
  private mapLocalBotApiFilePath(filePath: string): string | undefined {
    const serverRoot = process.env.TELEGRAM_LOCAL_FILE_SERVER_ROOT?.trim();
    const hostRoot = process.env.TELEGRAM_LOCAL_FILE_HOST_ROOT?.trim();
    if (!(serverRoot && hostRoot)) {
      return undefined;
    }
  ...
  private buildOutgoingMessage(
    ...
    if (typeof message === "object" && message !== null) {
      if ("markdown" in message) {
        return this.buildRichTextMessage(
          this.formatConverter.toAst(message.markdown)
        );
      }
